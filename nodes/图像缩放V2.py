...
class 图像缩放V2_孤海:
    ...
    def 执行缩放(self, 图像=None, 缩放方法="按长边等比例", 宽度=512, 高度=512,
              将边缩放到=1024, 缩放插值="Lanczos", 缩放模式="裁剪",
              固定方向="居中", 执行条件="总是", 填充颜色="#364254", 整除数=0,
              遮罩=None):
        if 图像 is not None:
            ...
            batch_size = 遮罩.shape[0]
            原高度, 原宽度 = 遮罩.shape[-2], 遮罩.shape[-1]
            # IMAGE 是固定输出端口；无图像时用黑色占位图保留遮罩处理能力。
            图像 = torch.zeros((batch_size, 原高度, 原宽度, 3), dtype=torch.float32)
        else:
            raise ValueError("图像和遮罩不能同时为空")
    ...
    def 处理单张图像(self, 图像, 遮罩, 缩放方法, 宽度, 高度, 将边缩放到, 缩放插值, 缩放模式, 固定方向, 执行条件, 填充颜色, 整除数):
        ...
        if 缩放方法 == "自定义宽高":
            if 宽度 != 0 and 高度 != 0:
                ...
                if 原宽度 >= 原高度:
                    ...
                    原图长边, 目标长边 = 原高度, 高度
                    原图短边, 目标短边 = 原宽度, 宽度
                if 执行条件 == "最长边大于时" and 原图长边 <= 目标长边:
                    需要缩放 = False
                elif 执行条件 == "最小边小于时" and 原图短边 >= 目标短边:
                    需要缩放 = False
            elif 宽度 != 0:
                if 执行条件 == "最长边大于时" and 原宽度 <= 宽度:
                    需要缩放 = False
                elif 执行条件 == "最小边小于时" and 原宽度 >= 宽度:
                    需要缩放 = False
            elif 高度 != 0:
                if 执行条件 == "最长边大于时" and 原高度 <= 高度:
                    需要缩放 = False
                elif 执行条件 == "最小边小于时" and 原高度 >= 高度:
                    需要缩放 = False
        else:
        ...
        def 就近整除(尺寸):
            """将尺寸取到最近的整除数倍数，至少保留一个整除数。"""
            if 整除数 <= 1:
                return 尺寸
            return max(整除数, int(math.floor(尺寸 / 整除数 + 0.5)) * 整除数)
        ...
        def 按模式整除(尺寸):
            ...
            if 缩放模式 == "拉伸":
                return 就近整除(尺寸)
            if 缩放模式 in ("裁剪", "总像素_等比例"):
                return max(整除数, (尺寸 // 整除数) * 整除数)
            return max(整除数, ((尺寸 + 整除数 - 1) // 整除数) * 整除数)

        def 处理等比例目标(图像宽, 图像高):
            """按选择的缩放模式处理已完成整除的等比例目标尺寸。"""
            if 缩放模式 == "拉伸":
                return self.调整尺寸(img, mask_pil, 图像宽, 图像高, 缩放插值)
            if 缩放模式 in ("裁剪", "总像素_等比例"):
                宽比 = 图像宽 / 原宽度
                高比 = 图像高 / 原高度
                覆盖比例 = max(宽比, 高比)
                覆盖宽度 = max(round(原宽度 * 覆盖比例), 图像宽)
                覆盖高度 = max(round(原高度 * 覆盖比例), 图像高)
                img_out, mask_out = self.调整尺寸(
                    img, mask_pil, 覆盖宽度, 覆盖高度, 缩放插值
                )
                左边, 顶边, 右边, 底边 = self.计算裁剪区域(
                    覆盖宽度, 覆盖高度, 图像宽, 图像高, 固定方向
                )
                return (
                    img_out.crop((左边, 顶边, 右边, 底边)),
                    mask_out.crop((左边, 顶边, 右边, 底边)),
                )
            if 缩放模式 == "填充_自定颜色":
                return self.智能填充(
                    img, mask_pil, 图像宽, 图像高, 缩放插值, 填充颜色, 固定方向
                )
            if 缩放模式 == "填充_边框颜色":
                return self.边框颜色填充(
                    img, mask_pil, 图像宽, 图像高, 缩放插值, 固定方向
                )
            return self.边缘像素填充(
                img, mask_pil, 图像宽, 图像高, 缩放插值, 固定方向
            )
        ...
        def 计算等比例目标(目标边, 按长边):
            """计算指定长边/短边取整后的另一边，并保持当前取整方向。"""
            # 指定的固定边始终就近取整，另一边再按缩放模式取整。
            目标边 = 就近整除(目标边)
            if 按长边:
                if 最长边 == 原宽度:
                    目标宽 = 目标边
                    目标高 = round(原高度 * (目标边 / 原宽度))
                else:
                    目标高 = 目标边
                    目标宽 = round(原宽度 * (目标边 / 原高度))
            else:
                if 最短边 == 原宽度:
                    目标宽 = 目标边
                    目标高 = round(原高度 * (目标边 / 原宽度))
                    ...
                    目标高 = 目标边
                    目标宽 = round(原宽度 * (目标边 / 原高度))
            return 按模式整除(目标宽), 按模式整除(目标高)

        def 自定义零尺寸处理():
            """宽高均为 0 时保留原尺寸，或按最近整除倍数适配。"""
            if 整除数 <= 1:
                return img, mask_pil, 原宽度, 原高度

            目标宽度 = 就近整除(原宽度)
            目标高度 = 就近整除(原高度)
            if (目标宽度, 目标高度) == (原宽度, 原高度):
                return img, mask_pil, 原宽度, 原高度
            if 缩放模式 == "拉伸":
                新图像, 新遮罩 = self.调整尺寸(img, mask_pil, 目标宽度, 目标高度, 缩放插值)
            elif 缩放模式 in ("裁剪", "总像素_等比例"):
                新图像, 新遮罩 = self.居中裁剪(
                    img, mask_pil, 目标宽度, 目标高度, 缩放插值, 固定方向
                )
            elif 缩放模式 == "填充_自定颜色":
                新图像, 新遮罩 = self.智能填充(
                    img, mask_pil, 目标宽度, 目标高度, 缩放插值, 填充颜色, 固定方向
                )
            elif 缩放模式 == "填充_边框颜色":
                新图像, 新遮罩 = self.边框颜色填充(
                    img, mask_pil, 目标宽度, 目标高度, 缩放插值, 固定方向
            ...
            return 新图像, 新遮罩, 目标宽度, 目标高度

        # 裁剪/填充模式用：取最近的整除数倍数，通过裁剪保持等比例
        ...
        if 需要缩放:
            if 缩放方法 == "按长边等比例":
                ...
                if 整除数 > 1:
                    新宽度, 新高度 = 计算等比例目标(将边缩放到, True)
                    img, mask_pil = 处理等比例目标(新宽度, 新高度)
                else:
            ...
            # 不需要缩放时，直接应用整除数要求
            新宽度, 新高度 = 原宽度, 原高度
            if 缩放方法 == "自定义宽高" and 宽度 == 0 and 高度 == 0:
                img, mask_pil, 新宽度, 新高度 = 自定义零尺寸处理()
            elif 整除数 > 1:
                整除宽度 = 就近整除(原宽度)
                整除高度 = 就近整除(原高度)
                ...
                if 整除宽度 > 原宽度 or 整除高度 > 原高度:
                    if 缩放模式 == "拉伸":
                        img, mask_pil = self.调整尺寸(img, mask_pil, 整除宽度, 整除高度, 缩放插值)
                    elif 缩放模式 in ("裁剪", "总像素_等比例"):
                        img, mask_pil = self.居中裁剪(
                            img, mask_pil, 整除宽度, 整除高度, 缩放插值, 固定方向
                        )
                    elif 缩放模式 == "填充_自定颜色":
                        img, mask_pil = self.智能填充(
                            img, mask_pil, 整除宽度, 整除高度, 缩放插值, 填充颜色, 固定方向
                        )
                    elif 缩放模式 == "填充_边框颜色":
                        img, mask_pil = self.边框颜色填充(
                            img, mask_pil, 整除宽度, 整除高度, 缩放插值, 固定方向
                        )
                    else:
                        img, mask_pil = self.边缘像素填充(
                            img, mask_pil, 整除宽度, 整除高度, 缩放插值, 固定方向
                        )
                    新宽度, 新高度 = 整除宽度, 整除高度
                elif 整除宽度 < 原宽度 or 整除高度 < 原高度:
    ...
    def 总像素等比例(self, img, mask, 目标宽度, 目标高度, 插值方法, 整除数):
        ...
        # 整除数处理：取最近倍数，通过裁剪保持等比例
        if 整除数 > 1:
            新宽度 = max(整除数, int(math.floor(新宽度 / 整除数 + 0.5)) * 整除数)
            新高度 = max(整除数, int(math.floor(新高度 / 整除数 + 0.5)) * 整除数)
...
class 长边短边:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "图像": ("IMAGE",),
                "宽度": ("INT", {"forceInput": True}),
                "高度": ("INT", {"forceInput": True}),
            },
        }

    RETURN_TYPES = ("INT", "INT", "FLOAT")
    RETURN_NAMES = ("长边", "短边", "比值")
    FUNCTION = "计算"
    CATEGORY = "孤海工具箱"
    ...
    def 计算(self, 图像=None, 宽度=None, 高度=None):
        try:
            图像宽度 = 图像高度 = None
            单值输入 = False
            if 图像 is not None:
                try:
                    形状 = tuple(图像.shape)
                    if len(形状) >= 3:
                        图像高度 = self._安全整数(形状[-3])
                        图像宽度 = self._安全整数(形状[-2])
                except (AttributeError, TypeError, ValueError, IndexError):
                    pass

            if 图像宽度 is not None and 图像高度 is not None:
                最终宽度, 最终高度 = 图像宽度, 图像高度
            else:
                输入宽度 = self._安全整数(宽度)
                输入高度 = self._安全整数(高度)
                if 输入宽度 is None and 输入高度 is None:
                    最终宽度 = 最终高度 = 0
                elif 输入宽度 is None:
                    最终宽度 = 最终高度 = 输入高度
                    单值输入 = True
                    ...
                    最终宽度 = 最终高度 = 输入宽度
                    单值输入 = True
                elif 输入宽度 == 0 and 输入高度 > 0:
                    最终宽度 = 最终高度 = 输入高度
                    单值输入 = True
                elif 输入高度 == 0 and 输入宽度 > 0:
                    最终宽度 = 最终高度 = 输入宽度
                    单值输入 = True
                else:
                    最终宽度, 最终高度 = 输入宽度, 输入高度
            ...
            长边 = max(最终宽度, 最终高度)
            短边 = min(最终宽度, 最终高度)
            比值 = float(最终宽度 / 最终高度) if 最终高度 and not 单值输入 else 0.0
            return (int(长边), int(短边), 比值)
        except Exception:
            # 节点作为尺寸兜底工具时，任何异常都返回稳定的零值。
            return (0, 0, 0.0)
